...
* ~~להוסיף Cron~~ — **עודכן 12/8: Cron כבר קיים ורץ בפרודקשן (Render).** הכלל הנותר: אין לשנות את מנגנון ה-Cron/הנעילה בלי אישור מפורש.
...
* ~~שימוש ב-Prisma לגישה ל-Database~~ — **עודכן 12/8: אין Prisma בפרויקט בפועל.** הגישה למסד הנתונים היא ישירה עם `@supabase/supabase-js`. ראו `docs/DATABASE.md`.
...
* כל שינוי ב-Database חייב להיות מוגדר קודם במסמך `docs/DATABASE.md` (לא `05-database.md` — הקובץ הזה לא קיים).
* אין להוסיף טבלאות בלי אישור.
* אין למחוק עמודות בלי אישור.
* אין לשנות שמות עמודות בלי אישור.
* ~~יש להשתמש ב-Prisma בלבד~~ — **עודכן 12/8**: אין Prisma. הגישה היחידה בפועל היא `@supabase/supabase-js` ישירות. שינויי סכמה מתועדים כ-SQL ידני תחת `docs/project/migrations/`.
* אין לכתוב SQL ישיר בקוד האפליקציה אלא אם יש סיבה מיוחדת ואישור (migrations תחת `docs/project/migrations/` הם כן SQL ישיר, וזה מקובל).
